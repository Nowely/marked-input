...
| [36](36-published-surface-leftovers.md) | Grip `aria-label`, `Store`'s rename TODO, `RowProps.index` | resolved | "Row options" announced; `Store` keeps its name; `index` kept — as a sibling position, not the ordinal first published. SUPERSEDED 2026-08-29: `index` removed in ADR-0013, measured at half of Enter's cost at 4000 rows |
...
| [45](45-a-split-repaints-every-row-after-it.md) | A structural row edit re-renders every sibling after it | resolved | Cause confirmed and its own discriminator refuted: dropping the sibling position cuts the slope by ~65%, not to zero. `RowProps.index` removed and the row `ref` stabilised (ADR-0013); 248 -> 117 ms, repaints 4001 -> 1. The residual is React's own keyed reconciliation |
| [46](46-vue-row-kind-has-no-reactive-node-read.md) | A Vue row kind has no reactive read of its own node | resolved | Neither candidate taken: the NODE is reactive now (ADR-0014), so a plain `computed(() => node.slot())` works and the incantation, its guide rule and the boundary hole all went. Cost is identity — compare rows by `id`, not `===` |
| [47](47-vue-costs-three-times-react-at-scale.md) | The Vue adapter costs three times React at document scale | resolved | Every row repainted on every edit, wherever the caret was: `syncProps` rebuilt two of `slots.node`'s five inputs inline, one recompute woke 4000 watchers. Both derived once now — 820 -> 37.5 ms, repaints 4007 -> 0, pinned in both positions |
