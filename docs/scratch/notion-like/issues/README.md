...
| [47](47-vue-costs-three-times-react-at-scale.md) | The Vue adapter costs three times React at document scale | needs-triage | Filed out of 45's unmeasured Vue half: 820 ms against React's 248 for the same Enter, and Vue's cost barely depends on WHERE the caret is — so it is a different defect, not a Vue-flavoured copy. Cause unmeasured; a render counter is not the instrument |
