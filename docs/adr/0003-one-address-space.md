...
Amending it is still a contract change. Two things this check deliberately does not cover: the adapters, where `TreeNode.position` is published API a consumer may read, and absolute offsets that never touch `.position` — `TokenHandle.caretIndex()` is a DOM-space measure of a bound element rather than a position in the value, so the scan would not see it either way. `keyboard/blockEdit.ts` was its last caller and no longer has one: the `placeCaret(0 | Infinity)` half went with `focusRow` when the row merge moved onto anchors, and the read half went with `rowEdgeAnchors`. Nothing above `features/tokens/` forms an absolute offset today.
