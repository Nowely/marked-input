...
describe('ParseFeature', () => {
    ...
    describe('sync()', () => {
        ...
        it('falls back to defaultValue when value is undefined', () => {
            ...
            expect(store.state.tokens.get()).toEqual([{type: 'text', content: 'default', position: {start: 0, end: 7}}])
        ...
        it('falls back to empty string when both are undefined', () => {
            ...
            expect(store.state.tokens.get()).toEqual([{type: 'text', content: '', position: {start: 0, end: 0}}])
        ...
        it('skips markup when no Mark override and no per-option Mark', () => {
            ...
            expect(store.state.tokens.get()).toEqual([{type: 'text', content: '@hello', position: {start: 0, end: 6}}])
    ...
    describe('parse handler', () => {
        it('in recovery mode — re-parses from token text', () => {
            ...
            expect(store.state.tokens.get()).toEqual([{type: 'text', content: 'test', position: {start: 0, end: 4}}])
            expect(store.state.previousValue.get()).toBe('test')
