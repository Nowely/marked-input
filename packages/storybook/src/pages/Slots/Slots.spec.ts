...
describe('Slots API', () => {
	describe('Container slot', () => {
		it('use default div component when no slot is provided', async () => {
			const {host} = await mountComponent({separator: null, value: VALUE})
		...
		it('use custom component from slots.container', async () => {
			const {host} = await mountComponent({separator: null, value: VALUE, slots: {container: CustomContainer}})
		...
		it('compose user slotProps.container ref (function) with host ref — editor still renders', async () => {
			...
			const {host} = await mountComponent({separator: null, value: VALUE, slotProps: {container: {ref: userRef}}})
	...
	describe('Span slot', () => {
		it('use default span component when no slot is provided', async () => {
			await mountComponent({separator: null, value: VALUE})
		...
		it('use custom component from Span prop', async () => {
			const {host} = await mountComponent({separator: null, value: VALUE, Span: CustomSpan})
		...
		it('apply custom className via custom Span component', async () => {
			await mountComponent({
				separator: null,
				value: VALUE,
				Span: defineMark({tag: 'span', class: 'custom-span-class'}),
			})
	...
	describe('contentEditable topology', () => {
		...
		it('have contentEditable="false" on the container when readOnly is true', async () => {
			const {host} = await mountComponent({separator: null, value: VALUE, readOnly: true})
		...
		it('freeze a value-only mark root as an atomic', async () => {
			await mountComponent({separator: null, Mark, value: 'Hello @[world](1)'})
	...
	describe('Custom slot components', () => {
		...
		it('allow native HTML elements as container slot', async () => {
			const {host} = await mountComponent({separator: null, value: VALUE, slots: {container: 'article'}})
	...
	describe('Edge cases', () => {
		it('handle empty value', async () => {
			const {host} = await mountComponent({separator: null, value: ''})
		...
		it('handle undefined slotProps gracefully', async () => {
			const {host} = await mountComponent({separator: null, value: VALUE, slotProps: undefined})
