...
describe('Clipboard: copy', () => {
	...
	it('selecting text + mark via drag should set markput MIME', async () => {
		...
		const helloNode = textNodes.find(n => n.textContent.startsWith('hello'))
		const worldNode = textNodes.find(n => n.textContent === 'world')
		if (!helloNode || !worldNode) throw new Error('expected text nodes not found')

		// Select from "hello " start to "world" end (span + mark)
		setSelection(helloNode, 0, worldNode, worldNode.length)
