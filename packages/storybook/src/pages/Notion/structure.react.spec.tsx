...
describe('every kind the showcase declares', () => {
	...
	it('types the title from "@title " and keeps only its body', () => {
		expect(tree('@title Apollo — Q2 launch plan')).toMatchInlineSnapshot(`
			"0: ROW "@title Apollo — Q2 launch plan" [0-30] kind=2
	...
	it('types a caption from "@caption "', () => {
		expect(tree('@caption Inline database · 24 items')).toMatchInlineSnapshot(`
			"0: ROW "@caption Inline database · 24 items" [0-35] kind=3
	...
	it('keeps a properties body raw, separators and all', () => {
		expect(tree('@properties\nStatus: chip:amber:In progress\nOwner: <who:Kara Vance>\n@end'))
			...
			"0: ROW "@properties↲Status: chip:amber:In progress↲Owner: <who:Kara Vance>↲@end" [0-71] kind=4
	...
	it('keeps the table of contents raw, indent and all', () => {
		expect(tree('@toc\nLaunch tasks\n\tSprint board <status:At risk>\n@end')).toMatchInlineSnapshot(`
			"0: ROW "@toc↲Launch tasks↲⇥Sprint board <status:At risk>↲@end" [0-53] kind=6
	...
	it('types "## " as heading 2', () => {
		expect(tree('## Launch tasks')).toMatchInlineSnapshot(`
			"0: ROW "## Launch tasks" [0-15] kind=8
	...
	it('types "> " as a quote and nests an indented line under it', () => {
		expect(tree("> If the cutover isn't boring, we're not ready to call it GA.\n\tSaid at the GA review."))
			.toMatchInlineSnapshot(`
			"0: ROW "> If the cutover isn't boring, we're not ready to call it GA.↲⇥Said at the GA review." [0-85] kind=10
	...
	it('reads the callout tone into meta and leaves the plain quote alone', () => {
		expect(tree('> [!danger] Launch gating on the auth migration\n> If the cutover is boring.'))
			.toMatchInlineSnapshot(`
			"0: ROW "> [!danger] Launch gating on the auth migration↲" [0-48] kind=11 meta="danger"
				0.0: TEXT "Launch gating on the auth migration" [12-47]
			 1: ROW "> If the cutover is boring." [48-75] kind=10
	...
	it('keeps a code body raw and reads its language into meta', () => {
		expect(tree('```bash\napollo deploy --env=staging --canary=5%\n# → rollout 5% · ==healthy==\n```'))
			.toMatchInlineSnapshot(`
			"0: ROW "\`\`\`bash↲apollo deploy --env=staging --canary=5%↲# → rollout 5% · ==healthy==↲\`\`\`" [0-80] kind=12 meta="bash"
	...
	it('nests an indented bullet under the bullet above it', () => {
		expect(tree('- EU region capacity unconfirmed\n\t- Awaiting quota approval\n- Support headcount at 60%'))
			.toMatchInlineSnapshot(`
			"0: ROW "- EU region capacity unconfirmed↲⇥- Awaiting quota approval↲" [0-60] kind=13
				0.0: TEXT "EU region capacity unconfirmed" [2-32]
				0.1: ROW "⇥- Awaiting quota approval↲" [33-60] lead="⇥" kind=13
					0.1.0: TEXT "Awaiting quota approval" [36-59]
			 1: ROW "- Support headcount at 60%" [60-86] kind=13
	...
	it('types "1. " as a numbered item', () => {
		expect(tree('1. Auth migration owns the critical path.')).toMatchInlineSnapshot(`
			"0: ROW "1. Auth migration owns the critical path." [0-41] kind=14
	...
	it('reads a to-do box into meta and beats the bullet opener', () => {
		expect(tree('- [ ] Confirm the EU quota with the vendor\n- [x] Signed off by Platform')).toMatchInlineSnapshot(`
			"0: ROW "- [ ] Confirm the EU quota with the vendor↲" [0-43] kind=15 meta=" "
				0.0: TEXT "Confirm the EU quota with the vendor" [6-42]
			 1: ROW "- [x] Signed off by Platform" [43-71] kind=15 meta="x"
	...
	it('nests a closed toggle body under its own line', () => {
		expect(tree('▸ Single-region GA first\n\tEU capacity is unconfirmed.')).toMatchInlineSnapshot(`
			"0: ROW "▸ Single-region GA first↲⇥EU capacity is unconfirmed." [0-53] kind=16
	...
	it('types the open toggle as its own kind and nests its children', () => {
		expect(
			...
			"0: ROW "▾ Why we cut the Android target↲⇥Shipping three platforms at once.↲⇥1. Auth migration owns the critical path." [0-109] kind=17
				...
				0.2: ROW "⇥1. Auth migration owns the critical path." [67-109] lead="⇥" kind=14
	...
	it('leaves a bullet indented under a blank line a root, since an empty row takes no children', () => {
		expect(tree('- alpha\n\n\t- beta')).toMatchInlineSnapshot(`
			"0: ROW "- alpha↲" [0-8] kind=13
				0.0: TEXT "alpha" [2-7]
			 1: ROW "↲" [8-9]
				1.0: TEXT "" [8-8]
			 2: ROW "⇥- beta" [9-16] lead="⇥" kind=13
				2.0: TEXT "beta" [12-16]"
		`)
	})
	...
	it('reads a lead that fills the whole line, so a blank continuation still nests', () => {
		expect(tree('- alpha\n\t\n\t\t- beta')).toMatchInlineSnapshot(`
			"0: ROW "- alpha↲⇥↲⇥⇥- beta" [0-18] kind=13
				0.0: TEXT "alpha" [2-7]
				0.1: ROW "⇥↲⇥⇥- beta" [8-18] lead="⇥"
					0.1.0: TEXT "" [9-9]
					0.1.1: ROW "⇥⇥- beta" [10-18] lead="⇥⇥" kind=13
	...
	it('carves a table line into cells at " | " and parses each cell as inline content', () => {
		expect(tree('| Auth service migration | <status:Blocked> | <who:Kara Vance> | <due:2026-04-02> | <bar:0.2>'))
			.toMatchInlineSnapshot(`
				"0: ROW "| Auth service migration | <status:Blocked> | <who:Kara Vance> | <due:2026-04-02> | <bar:0.2>" [0-93] kind=21
					0.0: ROW "Auth service migration" [2-24] kind=18
						0.0.0: TEXT "Auth service migration" [2-24]
					0.1: ROW " | <status:Blocked>" [24-43] lead=" | " kind=18
						0.1.0: TEXT "" [27-27]
						0.1.1: MARK "<status:Blocked>" [27-43] [value="Blocked"]
						0.1.2: TEXT "" [43-43]
					0.2: ROW " | <who:Kara Vance>" [43-62] lead=" | " kind=18
						0.2.0: TEXT "" [46-46]
						0.2.1: MARK "<who:Kara Vance>" [46-62] [value="Kara Vance"]
						0.2.2: TEXT "" [62-62]
					0.3: ROW " | <due:2026-04-02>" [62-81] lead=" | " kind=18
						0.3.0: TEXT "" [65-65]
						0.3.1: MARK "<due:2026-04-02>" [65-81] [value="2026-04-02"]
						0.3.2: TEXT "" [81-81]
					0.4: ROW " | <bar:0.2>" [81-93] lead=" | " kind=18
						0.4.0: TEXT "" [84-84]
	...
	it('carves the header line into header cells', () => {
		expect(tree('|= Task | Status | Owner | Due | Effort')).toMatchInlineSnapshot(`
			"0: ROW "|= Task | Status | Owner | Due | Effort" [0-39] kind=20
				0.0: ROW "Task" [3-7] kind=19
					0.0.0: TEXT "Task" [3-7]
				0.1: ROW " | Status" [7-16] lead=" | " kind=19
					0.1.0: TEXT "Status" [10-16]
				0.2: ROW " | Owner" [16-24] lead=" | " kind=19
					0.2.0: TEXT "Owner" [19-24]
				0.3: ROW " | Due" [24-30] lead=" | " kind=19
					0.3.0: TEXT "Due" [27-30]
				0.4: ROW " | Effort" [30-39] lead=" | " kind=19
	...
	it('types "|+ " as the footer and carves nothing', () => {
		expect(tree('|+ Count 24 · 9 done')).toMatchInlineSnapshot(`
			"0: ROW "|+ Count 24 · 9 done" [0-20] kind=22
	...
	it('keeps the view bar as one body the tabs are read from', () => {
		expect(tree('@views Table|Board|Timeline|Calendar')).toMatchInlineSnapshot(`
			"0: ROW "@views Table|Board|Timeline|Calendar" [0-36] kind=23
	...
	it('keeps the board a single row with a raw body', () => {
		expect(
			...
			"0: ROW "@board↲To do↲- Sign the vendor SLA <due:2026-04-09>|red:Legal↲Shipped↲- Beta invites|green:Growth↲@end" [0-102] kind=24
	...
	it('keeps the metric cards a single row with a raw body', () => {
		expect(tree('@metrics\nBeta users|4,120\np95 latency|184ms\nRollout|<bar:0.6>\n@end')).toMatchInlineSnapshot(`
			"0: ROW "@metrics↲Beta users|4,120↲p95 latency|184ms↲Rollout|<bar:0.6>↲@end" [0-66] kind=25
	...
	it('splits a bookmark into meta and title', () => {
		expect(
			...
			"0: ROW "@bookmark(https://example.com/apollo|How the auth migration changes token lifetimes.) Auth migration — rollout plan" [0-115] kind=26 meta="https://example.com/apollo|How the auth migration changes token lifetimes."
	...
	it('keeps a comment thread a single row with a raw body', () => {
		expect(
			...
			"0: ROW "@comments↲Kara Vance|2h ago|Can @[Platform](team-platform) confirm the EU quota?↲Milo Freeman|41m ago|Asked this morning.↲@end" [0-126] kind=27
	...
	it('parses something with every option the showcase declares', () => {
		const missing = vocabulary.notionOptions.filter(option => !parsed.has(indexOf(option))).map(nameOf)
	...
	it('parses to one known shape', () => {
		expect(tree(APOLLO_DOC)).toMatchInlineSnapshot(`
			"0: ROW "@title Apollo — Q2 launch plan↲" [0-31] kind=2
			 ...
			 4: ROW "@toc↲Launch tasks↲⇥Sprint board↲⇥Metrics & risks↲Decision log↲@end↲" [538-605] kind=6
				4.0: TEXT "Launch tasks↲⇥Sprint board↲⇥Metrics & risks↲Decision log" [543-599]
			 5: ROW "## Launch tasks↲" [605-621] kind=8
				5.0: TEXT "Launch tasks" [608-620]
			 6: ROW "@caption Inline database · 24 items↲" [621-657] kind=3
				6.0: TEXT "Inline database · 24 items" [630-656]
			 7: ROW "@views Table|Board|Timeline|Calendar↲" [657-694] kind=23
				7.0: TEXT "Table|Board|Timeline|Calendar" [664-693]
			 8: ROW "|= Task | Status | Owner | Due | Effort↲" [694-734] kind=20
				8.0: ROW "Task" [697-701] kind=19
					8.0.0: TEXT "Task" [697-701]
				8.1: ROW " | Status" [701-710] lead=" | " kind=19
					8.1.0: TEXT "Status" [704-710]
				8.2: ROW " | Owner" [710-718] lead=" | " kind=19
					8.2.0: TEXT "Owner" [713-718]
				8.3: ROW " | Due" [718-724] lead=" | " kind=19
					8.3.0: TEXT "Due" [721-724]
				8.4: ROW " | Effort" [724-733] lead=" | " kind=19
					8.4.0: TEXT "Effort" [727-733]
			 9: ROW "| Auth service migration | <status:Blocked> | <who:Kara Vance> | <due:2026-04-02> | <bar:0.2>↲" [734-828] kind=21
				9.0: ROW "Auth service migration" [736-758] kind=18
					9.0.0: TEXT "Auth service migration" [736-758]
				9.1: ROW " | <status:Blocked>" [758-777] lead=" | " kind=18
					9.1.0: TEXT "" [761-761]
					9.1.1: MARK "<status:Blocked>" [761-777] [value="Blocked"]
					9.1.2: TEXT "" [777-777]
				9.2: ROW " | <who:Kara Vance>" [777-796] lead=" | " kind=18
					9.2.0: TEXT "" [780-780]
					9.2.1: MARK "<who:Kara Vance>" [780-796] [value="Kara Vance"]
					9.2.2: TEXT "" [796-796]
				9.3: ROW " | <due:2026-04-02>" [796-815] lead=" | " kind=18
					9.3.0: TEXT "" [799-799]
					9.3.1: MARK "<due:2026-04-02>" [799-815] [value="2026-04-02"]
					9.3.2: TEXT "" [815-815]
				9.4: ROW " | <bar:0.2>" [815-827] lead=" | " kind=18
					9.4.0: TEXT "" [818-818]
					9.4.1: MARK "<bar:0.2>" [818-827] [value="0.2"]
					9.4.2: TEXT "" [827-827]
			 10: ROW "| Realtime sync engine | <status:In progress> | <who:Milo Freeman> | <due:2026-04-18> | <bar:0.6>↲" [828-926] kind=21
				10.0: ROW "Realtime sync engine" [830-850] kind=18
					10.0.0: TEXT "Realtime sync engine" [830-850]
				10.1: ROW " | <status:In progress>" [850-873] lead=" | " kind=18
					10.1.0: TEXT "" [853-853]
					10.1.1: MARK "<status:In progress>" [853-873] [value="In progress"]
					10.1.2: TEXT "" [873-873]
				10.2: ROW " | <who:Milo Freeman>" [873-894] lead=" | " kind=18
					10.2.0: TEXT "" [876-876]
					10.2.1: MARK "<who:Milo Freeman>" [876-894] [value="Milo Freeman"]
					10.2.2: TEXT "" [894-894]
				10.3: ROW " | <due:2026-04-18>" [894-913] lead=" | " kind=18
					10.3.0: TEXT "" [897-897]
					10.3.1: MARK "<due:2026-04-18>" [897-913] [value="2026-04-18"]
					10.3.2: TEXT "" [913-913]
				10.4: ROW " | <bar:0.6>" [913-925] lead=" | " kind=18
					10.4.0: TEXT "" [916-916]
					10.4.1: MARK "<bar:0.6>" [916-925] [value="0.6"]
					10.4.2: TEXT "" [925-925]
			 11: ROW "| Pricing page rewrite | <status:Done> | <who:Ines Duarte> | <due:2026-03-27 done> | <bar:1>↲" [926-1019] kind=21
				11.0: ROW "Pricing page rewrite" [928-948] kind=18
					11.0.0: TEXT "Pricing page rewrite" [928-948]
				11.1: ROW " | <status:Done>" [948-964] lead=" | " kind=18
					11.1.0: TEXT "" [951-951]
					11.1.1: MARK "<status:Done>" [951-964] [value="Done"]
					11.1.2: TEXT "" [964-964]
				11.2: ROW " | <who:Ines Duarte>" [964-984] lead=" | " kind=18
					11.2.0: TEXT "" [967-967]
					11.2.1: MARK "<who:Ines Duarte>" [967-984] [value="Ines Duarte"]
					11.2.2: TEXT "" [984-984]
				11.3: ROW " | <due:2026-03-27 done>" [984-1008] lead=" | " kind=18
					11.3.0: TEXT "" [987-987]
					11.3.1: MARK "<due:2026-03-27 done>" [987-1008] [value="2026-03-27 done"]
					11.3.2: TEXT "" [1008-1008]
				11.4: ROW " | <bar:1>" [1008-1018] lead=" | " kind=18
					11.4.0: TEXT "" [1011-1011]
					11.4.1: MARK "<bar:1>" [1011-1018] [value="1"]
					11.4.2: TEXT "" [1018-1018]
			 12: ROW "| Load test at 5× peak | <status:Planned> | <who:Priya Raman> | <due:2026-05-06> | <bar:0>↲" [1019-1110] kind=21
				12.0: ROW "Load test at 5× peak" [1021-1041] kind=18
					12.0.0: TEXT "Load test at 5× peak" [1021-1041]
				12.1: ROW " | <status:Planned>" [1041-1060] lead=" | " kind=18
					12.1.0: TEXT "" [1044-1044]
					12.1.1: MARK "<status:Planned>" [1044-1060] [value="Planned"]
					12.1.2: TEXT "" [1060-1060]
				12.2: ROW " | <who:Priya Raman>" [1060-1080] lead=" | " kind=18
					12.2.0: TEXT "" [1063-1063]
					12.2.1: MARK "<who:Priya Raman>" [1063-1080] [value="Priya Raman"]
					12.2.2: TEXT "" [1080-1080]
				12.3: ROW " | <due:2026-05-06>" [1080-1099] lead=" | " kind=18
					12.3.0: TEXT "" [1083-1083]
					12.3.1: MARK "<due:2026-05-06>" [1083-1099] [value="2026-05-06"]
					12.3.2: TEXT "" [1099-1099]
				12.4: ROW " | <bar:0>" [1099-1109] lead=" | " kind=18
					12.4.0: TEXT "" [1102-1102]
					12.4.1: MARK "<bar:0>" [1102-1109] [value="0"]
					12.4.2: TEXT "" [1109-1109]
			 13: ROW "| Vendor SLA sign-off | <status:At risk> | <who:Tomas Alvarez> | <due:2026-04-09> | <bar:0.35>↲" [1110-1205] kind=21
				13.0: ROW "Vendor SLA sign-off" [1112-1131] kind=18
					13.0.0: TEXT "Vendor SLA sign-off" [1112-1131]
				13.1: ROW " | <status:At risk>" [1131-1150] lead=" | " kind=18
					13.1.0: TEXT "" [1134-1134]
					13.1.1: MARK "<status:At risk>" [1134-1150] [value="At risk"]
					13.1.2: TEXT "" [1150-1150]
				13.2: ROW " | <who:Tomas Alvarez>" [1150-1172] lead=" | " kind=18
					13.2.0: TEXT "" [1153-1153]
					13.2.1: MARK "<who:Tomas Alvarez>" [1153-1172] [value="Tomas Alvarez"]
					13.2.2: TEXT "" [1172-1172]
				13.3: ROW " | <due:2026-04-09>" [1172-1191] lead=" | " kind=18
					13.3.0: TEXT "" [1175-1175]
					13.3.1: MARK "<due:2026-04-09>" [1175-1191] [value="2026-04-09"]
					13.3.2: TEXT "" [1191-1191]
				13.4: ROW " | <bar:0.35>" [1191-1204] lead=" | " kind=18
					13.4.0: TEXT "" [1194-1194]
					13.4.1: MARK "<bar:0.35>" [1194-1204] [value="0.35"]
					13.4.2: TEXT "" [1204-1204]
			 14: ROW "|+ Count 24 · 9 done↲" [1205-1226] kind=22
				14.0: TEXT "Count 24 · 9 done" [1208-1225]
			 15: ROW "## Sprint board↲" [1226-1242] kind=8
				15.0: TEXT "Sprint board" [1229-1241]
			 16: ROW "@board↲To do↲- Sign the vendor SLA|red:Legal↲- EU region quota|blue:Infra↲- Launch copy review↲In progress↲- Auth migration|purple:Platform↲- p95 latency budget|amber:Perf↲Shipped↲- Beta invites|green:Growth↲@end↲" [1242-1455] kind=24
			 ...
			 17: ROW "## Metrics & risks↲" [1455-1474] kind=8
				17.0: TEXT "Metrics & risks" [1458-1473]
			 18: ROW "@metrics↲Beta users|4,120↲p95 latency|184ms↲Crash-free|99.4%↲Open bugs|37↲@end↲" [1474-1553] kind=25
				18.0: TEXT "Beta users|4,120↲p95 latency|184ms↲Crash-free|99.4%↲Open bugs|37" [1483-1547]
			 19: ROW "> [!danger] Launch gating on the auth migration — GA holds only if cutover lands by 2026-04-09.↲" [1553-1649] kind=11 meta="danger"
				19.0: TEXT "Launch gating on the auth migration — GA holds only if cutover lands by 2026-04-09." [1565-1648]
			 20: ROW "### Risks↲" [1649-1659] kind=9
				20.0: TEXT "Risks" [1653-1658]
			 21: ROW "- Vendor SLA unsigned↲" [1659-1681] kind=13
				21.0: TEXT "Vendor SLA unsigned" [1661-1680]
			 22: ROW "- EU region capacity unconfirmed↲⇥- Awaiting quota approval↲" [1681-1741] kind=13
				22.0: TEXT "EU region capacity unconfirmed" [1683-1713]
				22.1: ROW "⇥- Awaiting quota approval↲" [1714-1741] lead="⇥" kind=13
					22.1.0: TEXT "Awaiting quota approval" [1717-1740]
			 23: ROW "- Support headcount at 60%↲" [1741-1768] kind=13
				23.0: TEXT "Support headcount at 60%" [1743-1767]
			 24: ROW "- [ ] Confirm the EU quota with the vendor↲" [1768-1811] kind=15 meta=" "
				24.0: TEXT "Confirm the EU quota with the vendor" [1774-1810]
			 25: ROW "- [x] Signed off by Platform↲" [1811-1840] kind=15 meta="x"
				25.0: TEXT "Signed off by Platform" [1817-1839]
			 26: ROW "## Decision log↲" [1840-1856] kind=8
				26.0: TEXT "Decision log" [1843-1855]
			 27: ROW "▾ Why we cut the Android target↲⇥Shipping three platforms at once puts the auth migration on the critical path twice.↲⇥1. Auth migration owns the critical path.↲⇥1. Three platforms at once doubles the QA matrix.↲" [1856-2068] kind=17
				...
				27.2: ROW "⇥1. Auth migration owns the critical path.↲" [1974-2017] lead="⇥" kind=14
					27.2.0: TEXT "Auth migration owns the critical path." [1978-2016]
				27.3: ROW "⇥1. Three platforms at once doubles the QA matrix.↲" [2017-2068] lead="⇥" kind=14
					27.3.0: TEXT "Three platforms at once doubles the QA matrix." [2021-2067]
			 28: ROW "▸ Single-region GA first↲⇥EU capacity is unconfirmed, so a second region is a launch risk with no launch benefit.↲" [2068-2182] kind=16
			 ...
			 29: ROW "▸ Adopt CRDT over OT↲⇥Presence and offline edits fall out of the same merge; OT needed a server for each.↲" [2182-2288] kind=16
			 ...
			 30: ROW "## Canary procedure↲" [2288-2308] kind=8
				30.0: TEXT "Canary procedure" [2291-2307]
			 31: ROW "\`\`\`bash↲apollo deploy --env=staging --canary=5%↲# → rollout 5% · healthy · p95 184ms↲\`\`\`↲" [2308-2397] kind=12 meta="bash"
				31.0: TEXT "apollo deploy --env=staging --canary=5%↲# → rollout 5% · healthy · p95 184ms" [2316-2392]
			 32: ROW "> If the cutover isn't boring, we're not ready to call it GA.↲" [2397-2459] kind=10
				32.0: TEXT "If the cutover isn't boring, we're not ready to call it GA." [2399-2458]
			 33: ROW "@bookmark(https://example.com/apollo/auth-migration|How the auth migration changes token lifetimes, and what breaks if it slips.) Auth migration — rollout plan↲" [2459-2619] kind=26 meta="https://example.com/apollo/auth-migration|How the auth migration changes token lifetimes, and what breaks if it slips."
				33.0: TEXT "Auth migration — rollout plan" [2589-2618]
			 34: ROW "@comments↲Kara Vance|2h ago|Can we confirm the EU quota before Friday?↲Milo Freeman|41m ago|Asked the vendor this morning — expecting an answer tomorrow.↲@end↲" [2619-2778] kind=27
