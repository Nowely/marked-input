...
1. **Pinned variable web font + fixed metrics**, applied **only during tests** via `vitest.setup.ts` (live `storybook dev` / `storybook build` are unaffected and keep the real system-font look). A `<style>` tag forces `font-family: 'Inter Variable' !important`, `line-height: 1.5 !important`, and `text-rendering: geometricPrecision !important` on `*` (universal selector), plus `vertical-align: top !important` on `button, img, input, select, textarea, svg`. Five deliberate choices:
    ...
    - **Universal `*` selector** (not a whitelist like `.ant-tag, .rs-tag, …`) — a hand-picked list silently misses any new UI library added later (new AntD component, MUI Chip, custom wrapper). The test environment is isolated, so it's safe to blast `*` and guarantee full coverage with zero maintenance.
    - **No fallback family** (`'Inter Variable'` alone, **not** `'Inter Variable', sans-serif`) — a fallback hides font-load failures: if Inter Variable didn't load for any reason, Chromium would silently fall back to `-apple-system` / `DejaVu Sans`, reintroducing the exact drift we're eliminating. Without a fallback, a font-load regression produces an immediately visible baseline mismatch. `await document.fonts.ready` in the specs guarantees the font is actually loaded before each screenshot, so this is safe.
