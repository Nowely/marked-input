...
const overlayMatchRef = useMarkput(s => s.feature.overlay.overlayMatch) as Ref<OverlayMatch<Option> | undefined>
const overlayKey = computed(() => (overlayMatchRef.value ? store.key.get(overlayMatchRef.value.option) : undefined))
const resolveOverlay = useMarkput(s => s.feature.overlay.overlaySlot)
