...
### Issue: Hashtags with special characters

**Solution:** Sanitize hashtag text:
...
### Issue: Duplicate hashtags

**Solution:** Track used hashtags:

```tsx
const extractHashtags = (text: string): string[] => {
  const matches = text.match(/#\[([^\]]+)\]/g) || []
  const tags = matches.map((m) => m.match(/#\[([^\]]+)\]/)?.[1])
...
describe('HashtagEditor', () => {
  it('shows trending hashtags on #', async () => {
    const user = userEvent.setup()
    render(<HashtagEditor />)
