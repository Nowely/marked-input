...
**Problem:** Command executes twice when selected

**Solution:** Check that you're only handling the command once:

```tsx
const handleChange = (newValue: string) => {
  // ❌ Bad - checks every change
  if (newValue.includes('/[')) {
    executeCommand()
  }

  // ✅ Good - only checks new commands
  const prevCommands = value.match(/\/\[[^\]]+\]/g) || []
  const newCommands = newValue.match(/\/\[[^\]]+\]/g) || []

  if (newCommands.length > prevCommands.length) {
...
### Issue: Cursor position after command execution

**Problem:** Cursor jumps to wrong position after command

**Solution:** Manually set cursor position:
...
### Issue: Commands not filtering correctly

**Problem:** Search doesn't match all expected commands

**Solution:** Normalize search query:
...
export function searchCommands(query: string): Command[] {
  ...
  return COMMANDS.filter((cmd) =>
    normalizeString(cmd.label).includes(normalizedQuery) ||
    normalizeString(cmd.description).includes(normalizedQuery) ||
...
describe('SlashCommandEditor', () => {
  it('shows commands on /', async () => {
    const user = userEvent.setup()
    render(<SlashCommandEditor />)
...
- **[Mention System](./mention-system)** - Add @mentions
- **[Hashtags](./hashtags)** - Add #hashtag support
- **[Markdown Editor](./markdown-editor)** - Full Markdown support
- **[Configuration](../guides/configuration)** - Learn more patterns
