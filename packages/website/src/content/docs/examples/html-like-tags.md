...
**Problem:** `<color>text</size>` doesn't throw error

**Solution:** The two values pattern ensures opening and closing match:

```tsx
// Markup: '<__value__>__nested__</__value__>'
// This pattern REQUIRES both values to be identical
// <color>text</color> ✅
// <color>text</size> ❌ Won't match
```

### Issue: Attributes with spaces

**Problem:** `<link url="site name">` breaks parsing

**Solution:** Use URL encoding or escape quotes:

```tsx
const encodeAttribute = (value: string): string => {
  return value.replace(/\s/g, '%20').replace(/"/g, '%22')
}

// Usage: <link url="site%20name">
```

## Testing

```tsx
describe('HtmlLikeEditor', () => {
  it('renders color tag', () => {
    render(<HtmlLikeEditor />)

    const colorTag = screen.getByText('Blue text')
    expect(colorTag).toHaveStyle({ color: 'blue' })
  })

  it('supports nested tags', () => {
    const value = '<color=red>Red <size=20>Large</size></color>'
    render(<HtmlLikeEditor initialValue={value} />)

    const outerTag = screen.getByText(/Red/)
    expect(outerTag).toHaveStyle({ color: 'red' })
  })
})
```
...
- **[Nested Marks](../guides/nested-marks)** - Deep nesting patterns
- **[Configuration](../guides/configuration)** - Two values pattern
- **[Markdown Editor](./markdown-editor)** - Markdown tags
