...
- [Accessibility](#accessibility) - Screen reader and keyboard support
- [Testing](#testing) - How to test your mention system
...
export const MentionMark: FC<MentionProps> = ({
  username,
  userId,
  displayName,
  avatar,
...
export const MentionOverlay: FC<MentionOverlayProps> = ({ users }) => {
  ...
  if (filteredUsers.length === 0) {
    return (
      ...
      >
        <div className="mention-overlay-empty">
  ...
  return (
    ...
    >
      {filteredUsers.map((user, index) => (
        <button
          key={user.id}
          className={`mention-overlay-item ${
        ...
        >
          <img
            ...
            className="mention-overlay-avatar"
          />
    ...
    const fetchUsers = async () => {
      ...
      try {
        const response = await fetch(
          `/api/users/search?q=${encodeURIComponent(match.value)}`
        )
...
const GroupMentionMark: FC<{ name: string; memberCount: number }> = ({
  name,
  memberCount
}) => {
  return (
...
const MentionWithCard: FC<MentionProps> = (props) => {
  ...
  return (
    ...
    >
      @{props.username}
      {showCard && cardData && (
        <UserCard data={cardData} position="bottom" />
      )}
    </span>
  )
}
  ...
  return (
    <div>
      ...
      {mentionedUsers.length > 0 && (
        <p>{mentionedUsers.length} user(s) will be notified</p>
      )}
...
**Problem:** Mentions render but don't respond to clicks

**Solution:** Check that you're using a `<button>` or `<a>` element:
...
// ❌ Bad
<span onClick={handleClick}>@{username}</span> // Not keyboard accessible
```

### Issue: Overlay positioning incorrect

**Problem:** Overlay appears in wrong position

**Solution:** Ensure container has proper positioning:
...
### Issue: Special characters in usernames

**Problem:** Usernames with spaces or special chars break parsing

**Solution:** Use URL encoding or avoid pipes in metadata:

```tsx
slotProps: {
  mark: ({ value, meta }) => {
    const data = JSON.parse(decodeURIComponent(meta || '{}'))
    return {
      username: value,
      ...data
    }
...
### Issue: Performance with large user lists

**Problem:** Overlay sluggish with 1000+ users
...
```tsx
// Only send search query to server
const fetchUsers = async (query: string) => {
  const response = await fetch(`/api/users?q=${query}&limit=10`)
...
```tsx
// MentionEditor.test.tsx
import { render, screen, waitFor } from '@testing-library/react'
import userEvent from '@testing-library/user-event'
import { MentionEditor } from './MentionEditor'

describe('MentionEditor', () => {
  it('shows suggestions on @ character', async () => {
  ...
  it('inserts mention on selection', async () => {
    ...
    expect(screen.getByText('alice', { selector: '.mention-username' }))
      .toBeInTheDocument()
  })
})
```
...
- **[Slash Commands](./slash-commands)** - Build Notion-style /commands
- **[Hashtags](./hashtags)** - Add #hashtag support
- **[Configuration Guide](../guides/configuration)** - Learn more patterns
- **[Dynamic Marks](../guides/dynamic-marks)** - Advanced mark interactions
