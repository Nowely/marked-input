...
export function fuzzyFilter(items: AutocompleteItem[], query: string): AutocompleteItem[] {
    ...
    return items
        .map(item => ({
            item,
            score: Math.max(fuzzyScore(query, item.label), fuzzyScore(query, item.value)),
        }))
...
export const AutocompleteMark: FC<AutocompleteMarkProps> = ({value, category, icon}) => {
    return (
                        ...
                        {items.map(item => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {index < 9 && <kbd className="item-shortcut">{index + 1}</kbd>}
                                </button>
                            )
                        })}
                    </div>
