...
**Solution:** Debounce search and limit results:

```tsx
const debouncedSearch = useMemo(
  () => debounce((query: string) => {
    const results = fuzzyFilter(items, query).slice(0, 50)
...
describe('AutocompleteEditor', () => {
  it('filters items with fuzzy search', async () => {
    const { result } = renderHook(() =>
      useFuzzyFilter(items, 'alc')
    )
...
- **[Mention System](./mention-system)** - Learn @mention patterns
- **[Overlay Customization](../guides/overlay-customization)** - Advanced overlays
- **[Configuration](../guides/configuration)** - Multi-trigger setup
