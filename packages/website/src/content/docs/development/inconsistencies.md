...
| Enter inserts `\n`                | Enter is a newline **in the value**, not a DOM line break — the guard lowers `insertParagraph`/`insertLineBreak` to `'\n'`, so the browser never builds a `<div>`/`<br>` inside the host. In a document that parses into ROWS the keydown owns both keys instead, and the guard drops them: Enter splits the caret's row and Shift+Enter opens a continuation line under it. |
...
| Select-all, then type / paste / delete | Ctrl+A selects the editor's whole contents (one host, one boundary) and the next input replaces the **whole value**. Where the value parses into ROWS, Ctrl+A then Enter replaces the document with one fresh row — **BREAKING**, introduced by `603adfac`. |
