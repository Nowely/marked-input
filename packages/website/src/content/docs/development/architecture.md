...
Block row operations are NOT an event. `store.rows.action({...})` and its four-verb
...
class Store {
    ...
    readonly rows:      RowController      // Block layout for the whole editor: hover, drag, drop edge, menu
...
| **RowController**             | Block layout for the whole editor: the hovered/dragged row, the drop edge, the open menu, the row verbs the menu triggers, and the row geometry the layer paints at |
