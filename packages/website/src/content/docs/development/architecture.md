...
- Editable policy is the driver's, and it is now one host deep: `props.readOnly` writes the container's own `contenteditable`, and only `isUserSelecting` still reaches the per-surface `tokens.setEditable({editable, readOnly})` sweep.
...
- **Editable-state application** — `setEditable({editable, readOnly})` is called by the private selection driver whenever `isUserSelecting` changes; `readOnly` no longer routes through it, being the container write instead. bind applies the same state to newly mounted surfaces.
