...
keywords: [installation, setup, quick start, react component, vue component, mentions editor, autocomplete]
---
...
<Tabs syncKey="package-manager">
    ...
    ```bash
    # React
    yarn add @markput/react
    # Vue
    yarn add @markput/vue
    ```
	</TabItem>
	<TabItem label="bun" icon="bun">
    ```bash
    # React
    bun add @markput/react
    # Vue
    bun add @markput/vue
    ```
	</TabItem>
...
**Requirements**: React 19+ for `@markput/react`, Vue 3.5+ for `@markput/vue`.

The tutorial below uses React; the Vue API mirrors it — see the [Vue Storybook](https://markput-vue.vercel.app) for live Vue examples.
...
- **`markup`** — the pattern the choice is written as. A markup must not begin with a placeholder, so
  there is always a literal opener; `@[__value__]` turns `Alice` into `@[Alice]`
- **`overlay`** — configuration for the built-in `Suggestions` component:
...
3. When you select an item (e.g., `'Alice'`), Markput **writes** `@[Alice]` into the value and
   renders it through `Mark`
...
The editor above is already a multi-row document: `separator` defaults to `'\n'`, so every line is a
...
```tsx
import {MarkedInput, RowMenu} from '@markput/react'
...
That is the whole wiring for a `/` menu too: an option that declares a `menu` IS in it. Read
[Rows and Nesting](/guides/rows) next, then [Row Kinds](/guides/row-kinds).
