...
The `options` prop accepts an array of option objects. Each option configures how markup is parsed and how the overlay behaves:

- **`markup`** — the pattern used for parsing and inserting marks (same syntax as Step 1)
...
3. When you select an item (e.g., `'Alice'`), Markput **inserts** the markup. For simple strings, the item becomes both `value` (visible text) and `meta` (stored data)

The `Mark` component receives `MarkProps` with `value` and `meta` properties. You can import this type for better TypeScript support.
