...
| `Backspace` at a row's entry | Climbs the same demote ladder — depth first, then kind — for a row of ANY kind, which is where it parts from Enter: this key asks about the POSITION, Enter about the run. Otherwise the boundary expansion merges the row with the one above.                       |
...
| Click on a frozen block      | SELECTS that row. An atomic kind paints none of its own text, so there is no caret position in it; the selection is written across the row's own element, so the browser paints the block and `Backspace`, a paste and a drag act on it. A typed CHARACTER is refused there — see below. |
