...
| `Tab` / `Shift+Tab`          | Re-indents the row selection, or the caret's row, when the kind that owns the line declares `indents`. Inside a carved row it walks to the next/previous cell, and is consumed at the ends without moving the caret. Everywhere else the key leaves the field (ADR-0002). |
