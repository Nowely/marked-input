...
| `continues` | `false`     | Does this kind carry into the row a split produces. Enter at the end of a `continues` row opens another row of the same kind, with the same `meta`; mid-row, the tail keeps the kind. A list item continues, a heading does not, a checked to-do splits into two checked to-dos. |
