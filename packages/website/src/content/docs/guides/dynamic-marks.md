...
The node does not expose a DOM ref. React and Vue own structural DOM and register it privately with core through `store.refs`. Keyboard focus and caret placement are handled by `store.dom` and `store.selection`.
