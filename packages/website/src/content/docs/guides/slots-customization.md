...
```tsx
// ❌ Wrong
const CustomContainer = (props) => <div {...props} />
...
**Problem:** Custom props from `slotProps` not appearing

**Solution:** Spread all props:

```tsx
// ❌ Wrong
const CustomContainer = forwardRef((props, ref) => (
  <div ref={ref} className="custom" />  // Props not spread!
))
...
**Problem:** Styles from `slotProps` being ignored

**Solution:** Merge styles properly:

```tsx
// ❌ Wrong
const CustomContainer = forwardRef((props, ref) => (
  <div {...props} ref={ref} style={{ padding: '16px' }} />  // Overrides props.style!
))
...
**Problem:** Type errors with custom props

**Solution:** Extend the correct interface:
...
const CustomContainer = forwardRef<HTMLDivElement, CustomProps>(
  ({ customProp, ...props }, ref) => (
    <div {...props} ref={ref} data-custom={customProp} />
  )
)
```
...
**Problem:** Editor re-renders too often

**Solutions:**

```tsx
// Memoize custom components
const MemoContainer = memo(forwardRef((props, ref) => (
  <div {...props} ref={ref} />
)))
...
- **[Overlay Customization](./overlay-customization)** - Customize the suggestion overlay
- **[Keyboard Handling](./keyboard-handling)** - Advanced keyboard shortcuts
- **[API Reference](../api/components#markedinput)** - Complete slots API documentation
