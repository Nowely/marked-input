...
ParserV2 использует оптимизированный алгоритм парсинга на основе состояний кандидатов:

1. **Инициализация**: Создание дескрипторов разметки и группировка по триггерным символам
2. **Итеративный разбор**: Последовательная обработка текста через `processNextToken()`:
   - Поиск начала маркера на текущей позиции по триггерным символам
   - Для найденных маркеров используется `MarkupParseState` для отслеживания прогресса парсинга
   - **Состояния парсинга**: Фазы `start` → `middle` → `end` с индексами в паттернах
   - Маркер завершается только при полном соответствии всех паттернов
3. **Рекурсивная обработка маркеров**:
   ...
   - `children` содержат полную древовидную структуру (текст + вложенные маркеры)
