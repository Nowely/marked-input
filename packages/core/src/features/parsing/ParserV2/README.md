...
```
ParserV2/                           # Корень модуля
├── ParserV2.ts                    # Главный класс парсера
├── types.ts                       # Центральные типы и интерфейсы
├── index.ts                       # Экспорты API
├── README.md                      # Документация
├── core/                          # Основные классы парсера
│   ├── MarkupMatcher.ts           # Стратегия матчинга разметки
│   ├── PatternEngine.ts           # Движок паттернов с цепочками
│   └── MarkupDescriptor.ts        # Дескриптор разметки
├── utils/                         # Вспомогательные компоненты
│   ├── PatternBuilder.ts          # Строитель паттернов
│   ├── PatternChainManager.ts     # Менеджер цепочек паттернов
│   ├── SegmentMatcher.ts          # Матчинг сегментов
│   ├── TokenBuilder.ts            # Создание токенов
│   ├── AhoCorasick.ts             # Алгоритм Ахо-Корасик
│   └── validation.ts              # Функции валидации
├── ParserV2.spec.ts              # Unit тесты
├── ParserV2.integration.spec.ts  # Интеграционные тесты
└── benchmarks.ts                 # Бенчмарки производительности
```

**Организация по ответственности:**
- **`core/`** - основные классы, формирующие ядро парсера
- **`utils/`** - вспомогательные компоненты и алгоритмы
- **Корень** - публичное API, типы и документация

**Изменения после рефакторинга v2.3 (улучшение именования + организация файлов):**
...
- ✅ **Реорганизация структуры файлов**:
  - Создани папки `core/` и `utils/` для логической группировки
  - `core/` содержит основные классы парсера (MarkupMatcher, PatternEngine, MarkupDescriptor)
  - `utils/` содержит вспомогательные компоненты (PatternBuilder, TokenBuilder, validation, etc.)
...
- **Реорганизована структура файлов**: созданы папки `core/` и `utils/` для логической группировки
- Убраны утечки памяти и deprecated код
- Все 300+ тестов проходят ✅

### Достигнуто в v2.3 (Рефакторинг именования + организация файлов)
...
- ✅ **Переименование функций**:
  ...
  - `buildGuaranteedSequence` → `buildTokenSequence`
- ✅ **Реорганизация структуры файлов**:
  - Создани папки `core/` и `utils/` для логической группировки компонентов
  - `core/` содержит основные классы парсера (3 файла)
  - `utils/` содержит вспомогательные компоненты (6 файлов)
  - `ParserV2.ts` и `types.ts` остаются в корне как центральные сущности
- ✅ **Обновление импортов**: Все экспорты и импорты синхронизированы с новой структурой
- ✅ **Документация**: README полностью актуализирован с описанием новой архитектуры
