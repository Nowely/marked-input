...
- **BlockStore**: Per-row UI state (drag/hover/menu signals) and DOM wiring (`attachContainer`/`attachGrip`/`attachMenu`). One instance per row node, constructed by the controller with the shared `action` event and a live row-index reader (`tokens.rootIndexOf`), so the adapters attach bare elements and thread no index.
...
`operations.ts` holds one function and never takes a raw value. The document reaches it as a `SliceRead` — `(from, to) => tokens.valueBetween(from, to)` — so every read comes from the token tree itself and is consistent with the `rows` whose positions address it, which a props-first `value()` is not in controlled mode.

`addRowUnanchored(read, rows, afterIndex, separator)` is all that is left of the composed path — the two adds no anchor can name: an EMPTY tree has no row to insert after, and a negative `afterIndex` means before the first row, which `insertAfter` cannot express. Neither is reachable from the menu. The composition is two slices around the one cut the insertion names — the row-array projection (per-row texts and inter-row gaps) that used to back it is gone with its last consumer.
