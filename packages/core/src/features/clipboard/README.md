...
- **DOM selection as anchors**: Clipboard reads `store.tokens.domAnchors()` — what the live window selection says, resolved in the live tree. It forms no absolute offset (spec S2 D1); `store.tokens.valueBetween(anchor, head)` projects the span to a string.
