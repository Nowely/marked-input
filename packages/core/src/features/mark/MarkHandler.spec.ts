...
describe('MarkHandler', () => {
	it('return depth 0 for top-level mark', () => {
		...
		store.feature.parsing.tokens([{type: 'text', content: 'hi ', position: {start: 0, end: 3}}, markToken])
	...
	it('return parent as undefined for top-level mark', () => {
		const store = new Store()
		const markToken = createMarkToken()
		store.feature.parsing.tokens([markToken])
	...
	it('return depth 1 for nested mark', () => {
		...
		store.feature.parsing.tokens([outerMark])
	...
	it('return depth 0 when token not found in state', () => {
		const store = new Store()
		const markToken = createMarkToken()
		store.feature.parsing.tokens([{type: 'text', content: 'other', position: {start: 0, end: 5}}])
