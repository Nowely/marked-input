interface Difference extends DifferencePlus {
	aMove?: string[]
	aMoveIndex?: number[]
	bMove?: string[]
	bMoveIndex?: number[]
 ...
 *
 */
export function patienceDiff(aLines: string[], bLines: string[], diffPlusFlag?: boolean): Difference {
	// "Difference" is the array used to accumulate the aLines that are deleted, the
	...
	function recurseLCS(aLo: number, aHi: number, bLo: number, bHi: number, uniqueCommonMap?: Map<string, {
		...
		function addSubMatch(aLo: number, aHi: number, bLo:number, bHi: number) {
			...
			//
			// If not, add the subsequence to the Difference (all aLines having been
